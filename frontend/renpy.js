...
screen say(who, what):
    ...
    # ==========================================
    # 💡 UI 전체를 제어하는 마스터 변수 설정
    # ==========================================
    # 1. 기준 위치
    $ ui_x = 250           # UI 전체의 시작 X 좌표 (얼굴 박스 기준)
    $ ui_y = 780           # UI 전체의 시작 Y 좌표 (얼굴 박스와 대화상자의 윗부분 기준)
    
    # 2. 주인공 얼굴 박스
    $ face_size = 250      # 얼굴 박스의 가로/세로 크기
    $ gap = 30             # 얼굴 박스와 대화상자 사이의 가로 간격
    
    # 3. 대화 상자 (새로 추가됨!)
    $ tb_w = 1100          # 대화상자의 가로 길이
    $ tb_h = 250           # 대화상자의 세로 길이
    
    # 4. 네임박스 (새로 추가됨!)
    $ namebox_w = 200      # 네임박스의 가로 길이
    $ namebox_h = 60       # 네임박스의 세로 높이
    $ namebox_gap = 10      # 네임박스와 대화상자 사이의 세로 띄어쓰기 간격 (딱 붙이려면 0)

    # ------------------------------------------
    # ⚙️ 자동 계산 영역 (건드리지 않아도 됨!)
    # 대화상자와 네임박스의 시작 X 좌표: 기준점 + 얼굴크기 + 간격
...
# 1. 주인공 얼굴 박스 (제일 왼쪽)
    if SideImage() is not None:
        fixed:
            ...
            # 배경 박스 (네가 만든 둥근 테두리 이미지)
            add "gui/face_box.png" xysize(face_size, face_size) fit "fill"
            
            # 💡 파일 없이 코드로 모서리 깎기!
            add AlphaMask(
                child=SideImage(),
                # 둥근 모서리를 가진 가상의 검은색 판을 즉석에서 만듭니다.
                mask=Frame(Solid("#000"), corner_radius=30) 
            ) xysize(face_size - 20, face_size - 20) align(0.5, 0.5)
    ...
    window:
        ...
        xpos box_x          
        ypos ui_y          
        xsize tb_w         # 마스터 변수의 대화상자 가로 길이
        ysize tb_h         # 마스터 변수의 대화상자 세로 길이
        
        background Frame("gui/textbox.png", gui.textbox_borders, tile=False)

        # 대사 텍스트 출력 위치
        text what id "what":
            xpos 30       
            ypos 30       
            # 글씨가 대화상자를 뚫고 나가지 않게 (상자 가로 길이 - 여백 70)으로 자동 계산
            xsize (tb_w - 70) 
    ...
    if who is not None:
        window:
            ...
            # 계산된 변수 적용
            xpos box_x          
            # 대화상자 위치(ui_y)에서 네임박스 높이와 띄울 간격만큼 빼서 위로 올림!
            ypos ui_y - namebox_h - namebox_gap 
            
            # 마스터 변수의 네임박스 크기 적용
            xysize (namebox_w, namebox_h)   
            
            background Frame("gui/namebox.png", Borders(2, 2, 2, 2), tile=False) 
            padding (0, 0, 0, 0)

            text who id "who" align (0.5, 0.5) size 22

# ==========================================
    # 4. 퀵 메뉴 버튼 (대화상자 오른쪽 위 정렬)
    # ==========================================
    $ box_x = ui_x + face_size + gap 

    hbox:
        # 위치 설정: 대화상자 맨 오른쪽 끝에서 안쪽으로
        xpos box_x + tb_w
        xanchor 1.0  
        
        # 위치 Y 설정: 네임박스의 Y 위치와 완벽하게 동일하게 맞춤
        ypos ui_y - namebox_h - namebox_gap
        ...
        # 1. 되감기 버튼
        imagebutton:
            # 원본 이미지가 얼마나 크든 무조건 (namebox_h, namebox_h) 크기로 리사이징
            idle Transform("btn_rewind.png", size=(namebox_h, namebox_h))
            #hover Transform("btn_rewind_h.png", size=(namebox_h, namebox_h))
            action Rollback()          

        # 2. 세팅 버튼
        imagebutton:
            idle Transform("btn_setting.png", size=(namebox_h, namebox_h))
            #hover Transform("btn_setting_h.png", size=(namebox_h, namebox_h))
            action ShowMenu('preferences') 

        # 3. 메뉴로 돌아가기 버튼
        imagebutton:
            idle Transform("btn_menu.png", size=(namebox_h, namebox_h))
            #hover Transform("btn_menu_h.png", size=(namebox_h, namebox_h))
            action MainMenu()
...
style window:
    xalign 0.5
    xfill True  # <- 이 줄을 추가해 줘! (가로 폭을 화면에 꽉 차게 늘리라는 뜻이야)
    yalign 1.0
    ysize gui.textbox_height

    # 이 아래 background 코드는 그대로 두면 돼!
    background Frame("gui/textbox.png", gui.textbox_borders, tile=False)
style namebox:
    xpos gui.name_xpos
...
screen main_menu():
    ...
    # 1. 💡 배경 이미지를 1920x1080 해상도에 빈틈없이 꽉 채우는 마법의 명령어!
    add "background/bg_title.png" xysize (1920, 1080) fit "cover"
    ...
    vbox:
        ...
        # 3. 4개의 버튼
        textbutton "PLAY" action Start() xalign 0.5 text_size 60
        textbutton "LOAD" action ShowMenu("load") xalign 0.5 text_size 60
        textbutton "SETTING" action ShowMenu("preferences") xalign 0.5 text_size 60
        textbutton "EXIT" action Quit(confirm=not main_menu) xalign 0.5 text_size 60
